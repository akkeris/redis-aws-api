...
func provision(spec provisionspec, err binding.Errors, r render.Render) {
	...
	defer db.Close()
	var name string
	dberr = db.QueryRow("select name from provision where plan=$1 and claimed='no' and make_date=(select min(make_date) from provision where plan=$1 and claimed='no')", plan).Scan(&name)
	...
	if available {
		_, dberr = db.Exec("update provision set claimed='yes' where name=$1", name)
		if dberr != nil {
			fmt.Println(dberr)
			toreturn := dberr.Error()
			r.JSON(500, map[string]interface{}{"error": toreturn})
		...
		_, dberr = db.Exec("update provision set claimed='unavailable' where name=$1", name)
		if dberr != nil {
			fmt.Println(dberr)
			toreturn := dberr.Error()
			r.JSON(500, map[string]interface{}{"error": toreturn})
		...
		// try again, maybe one will be available this time. 
		provision(spec, err, r)
		return
	}
